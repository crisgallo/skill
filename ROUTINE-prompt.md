...
5. Notifica secondo il passo D di ROUTINE.md: ogni modifica ha un livello (🔴 sostanziale, 🟡 minore); mail il giorno stesso a gallinelli@gmail.com solo se c'è almeno una modifica sostanziale (oggetto "Sostanziale, GG/MM/AAAA: <nomi>"), più le minori in attesa da 14 giorni; il lunedì una mail unica di riepilogo con tutto quello che è in attesa (`snapshots/state.json`, chiave `pendenti`). Corpo come in ROUTINE.md: cosa è cambiato per ogni skill, link allo zip su GitHub, promemoria di eliminare la vecchia e caricare la nuova in Personalizza > Skill. Se Gmail non è disponibile, apri una issue su GitHub nel repository crisgallo/skill con lo stesso titolo e lo stesso corpo. Se nulla è cambiato, nessuna mail e nessuna issue. Se il giro fallisce per un errore che impedisce di completarlo, manda comunque una mail (o apri una issue) con oggetto "Giro skill del GG/MM/AAAA fallito" e la causa.

6. Prova di completamento, obbligatoria (passo E di ROUTINE.md): prima di chiudere esegui `git log origin/<ramo> -1` e `git show origin/<ramo>:reports/AAAA-MM-GG.md`. Se il commit di oggi e il report non sono sul remoto, il giro non è finito: commit e push finché non lo sono. Una sessione chiusa con file solo in staging è un giro fallito.
